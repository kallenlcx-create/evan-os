...
export async function runAiInsight(opts?: { limit?: number; force?: boolean }): Promise<{ results: InsightResult[]; ok: number; fail: number }> {
  const cfg = loadIntellectConfig()
  const limit = opts?.limit || cfg.aiInsightBatchMax || 30
  const customers = pickScope(await db.customers.toArray() as Customer[], cfg.aiInsightScope)
  ...
  const queue = customers.filter(c=>{
    ...
    if(!at) return true
    return now - new Date(at).getTime() > 86400000
  }).slice(0, limit)
...
{"profile_cn":"中文客户背景2-4句","intent":"high|medium|low","opportunity":"deal|repurchase|cross_sell|none","needs_followup":true/false,"followup_at":"YYYY-MM-DD可空","deal_hint":"一句话","confidence":0到1}`
    try{
      ...
      let tier: any = (c as any).aiTier || undefined
...
// ====== 已下单闭环：画像/周期/NBA（规则+轻量）=====
export async function runPurchaseLoop(){
  const orders = await listPurchaseOrders()
  ...
  for(const c of customers){
    ...
    if(!os.length && !tags.includes('已下单')) continue
    ...
    let buyerType = dates.length<=1 ? 'once' : (daysSince>420 ? 'dormant' : 'repeat')
    if(dates.length>=3 && daysSince<=cycle*1.5) buyerType='repeat'
    const products = [...new Set(os.flatMap(o=> o.products||[]))]
    const pattern = products.length<=1 ? 'same_product' : (dates.length>=2 ? 'multi_product' : 'unknown')
    const rec = new Set<string>()
    for(const p of products){
      for(const x of (CROSS[p]||[])) rec.add(x)
    }
    let nba: any = 'none'
    let nbaReason = ''
    ...
    const profile = `订单 ${os.length} 次 · 产品 ${products.join('/')||'—'} · 累计约 $${os.reduce((s,o)=>s+(o.amount||0),0)} · 距上次 ${daysSince} 天 · 周期约 ${cycle||'—'} 天`
    await db.customers.update(c.id, {
      ...
      nextBestAction: nba,
      nbaReason,
      aiProfile: (c as any).aiProfile || profile,
      purchaseIntelAt: new Date().toISOString(),
      ...(cfg.syncTierToFollowUps && nba==='reorder' && !suppressed ? { aiTier:'repurchase', aiReason: nbaReason } : {}),
    } as any)
    updated++
  }
  window.dispatchEvent(new CustomEvent('evan-customers-updated'))

      ...
      {activeSection === 'appearance' && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 space-y-5">
          ...
          <div className="space-y-4">
            ...
            {/* ====== 卡片外观（背景色 + 透明度）====== */}
            <div className="pt-4 border-t border-gray-100">
              <label className="text-sm font-medium text-gray-700 block mb-3">卡片外观</label>
              {/* 主题色选择 */}
              <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 mb-4">
                {CARD_THEMES.map(t => {
                  const cur = getCardTheme()
                  const active = cur.key === t.key
                  return (
                    <button
                      key={t.key}
              ...
              <div className="mb-3">
                <label className="text-[11px] text-gray-400 block mb-1">
                  卡片透明度（越大越透出壁纸）：<span className="text-gray-600">{Math.round(getCardTheme().opacity * 100)}%</span>
                </label>
                <input
                  type="range" min={0} max={100} step={5}
                  value={Math.round(getCardTheme().opacity * 100)}
                  onChange={e => { setCardThemeOpacity(Number(e.target.value) / 100); setCardVersion(v => v + 1) }}
                  className="w-full accent-blue-500"
                />
              </div>
              {/* 实时预览 */}
              <div
                className="h-12 rounded-xl border border-gray-200 flex items-center px-3 gap-2"
                style={{ backgroundColor: `rgba(${(() => { const h = getCardTheme().hex.replace('#',''); const n = parseInt(h,16); return `${(n>>16)&255},${(n>>8)&255},${n&255}` })()},${getCardTheme().opacity})` }}
              >
                <div className="w-2 h-2 rounded-full bg-blue-400" />
                <div className="h-1.5 w-16 bg-gray-300/50 rounded-full" />
                <span className="ml-auto text-[10px] text-gray-500">实时预览 · 透明度 {Math.round(getCardTheme().opacity * 100)}%</span>
              </div>
              <p className="text-[10px] text-gray-300 mt-1">全局卡片统一外观，透明度可叠加壁纸效果。</p>
            </div>
